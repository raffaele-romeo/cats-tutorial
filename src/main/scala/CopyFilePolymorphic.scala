...
object CopyFilePolymorphic extends IOApp {
  ...
  def transmit(origin: InputStream, destination: OutputStream, buffer: Array[Byte], acc: Long): IO[Long] =
    for {
      amount <- IO.delay(origin.read(buffer, 0, buffer.size))
      count  <- if(amount > -1) IO.delay(destination.write(buffer, 0, amount)) *> transmit(origin, destination, buffer, acc + amount)
      else IO.pure(acc) // End of read stream reached (by java.io.InputStream contract), nothing to write
    } yield count // Returns the actual amount of bytes transmitted

  def transfer(origin: InputStream, destination: OutputStream): IO[Long] =
    for {
      buffer <- IO.delay( new Array[Byte](1024 * 10) ) // Allocated only when F is evaluated
      total  <- transmit(origin, destination, buffer, 0L)
    } yield total

  def inputStream(f: File, guard: Semaphore[IO]): Resource[IO, FileInputStream] =
    Resource.make {
      IO.delay(new FileInputStream(f))
    } { inStream =>
      guard.withPermit {
  ...
  def outputStream(f: File, guard: Semaphore[IO]): Resource[IO, FileOutputStream] =
    Resource.make {
      IO.delay(new FileOutputStream(f))
    } { outStream =>
      guard.withPermit {
